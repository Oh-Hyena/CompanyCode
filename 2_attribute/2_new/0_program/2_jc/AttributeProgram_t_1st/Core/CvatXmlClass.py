...
from    abc    import * 
# 추상 클래스는 오로지 상속에만 사용한다. 그리고 파생 클래스에서 만드시 구현해야 할 메서드를 정해줄 때 사용한다.
...
class CvatXml(metaclass=ABCMeta):
    ...
    # 실제 CvatXml init 함수 (setInitCheckCondList() 을 통해 Default CheckCond List 를 CvatXml 생성시 바로 할당하는 함수)
    def initCvatXmlClass(self):
        self.condClass = CheckCondition(self.setInitCheckCondList())


    # cvatXml 파일들 리스트 추출하는 함수
    ...
    @abstractmethod   # 구현부가 없는 추상 메소드 선언
