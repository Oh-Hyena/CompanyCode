...
#img_augmentation할_xmlml
xmlRootpath = r"D:\HN_code\test\1_labeling\14_shearing\1_bus_img_xml\bus_xml"
#img_augmentation할_img
imageRootpath = r"D:\HN_code\test\1_labeling\14_shearing\1_bus_img_xml\bus_img"
#저장할폴더(txt파일)
rootpath=r"D:\HN_code\test\1_labeling\14_shearing\2_shearing_img_xml_txt"
# os.mkdir(rootpath)
targetpath=os.path.join(rootpath,'shearing_txt')
targetpath1=os.path.join(rootpath,'shearing_img')
save_xml=os.path.join(rootpath,'shearing_xml')
########txt저장
os.mkdir(targetpath)
os.mkdir(targetpath1)
os.mkdir(save_xml)
# videoclass = "UnitrainNight_unknowncar_shearing"

videoclass = "bus_shearing"
...
####################빈행렬나누기##########################
a = numpy.zeros(shape=(len(bndbox_count_shape)+500,200,4))
...
####################빈행렬나누기##########################
a1 = numpy.zeros(shape=(len(bndbox_count_shape)+500,200,1))
#########################################
...
for k,img in enumerate(total_images):
    cv_img = imread(img)[:,:,::-1]
    img_, bboxes_ = Shear(0.2)(cv_img.copy(), a[k].copy())
    cv2.imwrite("temp3."+mode, img_[:,:,::-1])
    os.rename("temp3."+mode, os.path.join(videoclass +"_"+str(fileCount)+"."+mode))
